...
client.once('ready', () => {
    console.log('Discord 이미지 봇이 준비되었습니다.');
});

const promptRegex = /^\s?gpt\s{0,3}```([pls])?\s?([\s\S]+)```/i;
...
client.on('messageCreate', async message => {
    ...
    if (
        ...
        if (queueList.length > 0) {
            ...
            queueList.forEach((item, idx) => {
                ...
                reply += `${idx + 1}. (${item.userMessage}) - ${secAgo}초 전${item.inProgress ? ' (진행중)' : ''}\n`;
            });
        }
        // 1000자 제한
    ...
    if (!promptRegex.test(message.content)) return;
    ...
    const matches = message.content.match(promptRegex);
    const [, sizeModifier, userMessage] = matches;

    const size = getImageSize(sizeModifier);
    const hasAttachments = message.attachments && message.attachments.size > 0;

    // 첨부파일이 있으면 i2i, 없으면 t2i로 요청
    try {
